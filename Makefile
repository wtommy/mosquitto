...
dist : reallyclean
	...
	cp -r client installer lib logo man misc security service src ChangeLog.txt CMakeLists.txt COPYING Makefile compiling.txt config.h config.mk external_security_checks.txt readme.txt mosquitto.conf aclfile.example pwfile.example dist/mosquitto-${VERSION}/
